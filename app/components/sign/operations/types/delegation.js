...
export default class OperationsPromptDelegation extends Component {
  ...
  render() {
    ...
    const totalVestsDPay = parseFloat(this.props.dpay.props.total_vesting_fund_dpay.split(" ")[0])
    const totalVests = parseFloat(this.props.dpay.props.total_vesting_shares.split(" ")[0])
    const bp = (vests > 0) ? (totalVestsDPay * vests / totalVests).toFixed(3) : 0
    ...
    if (this.props.account.vestingDelegations && this.props.account.vestingDelegations[opData.delegator]) {
      ...
      if(existingDelegation > 0) {
        const existingBp = (totalVestsDPay * existingDelegation / totalVests).toFixed(3)
        existingDelegationWarning = (
          <Grid.Row columns={1}>
            <Grid.Column>
              <Message info>
                A delegation of <strong>{existingBp} BP</strong> ({existingDelegation} VESTS) is already active from <AccountName name={opData.delegator}/> to <AccountName name={opData.delegatee}/>. This new delegation will <strong>overwrite</strong> the existing delegation (not add to it).
              </Message>
            </Grid.Column>
          </Grid.Row>
        )
