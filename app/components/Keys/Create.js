...
export default class KeysCreate extends Component {
  ...
  checkUsername = _.debounce((e, props) => {
    ...
    dpay.api.getAccounts([name], (err, results) => {
      this.setState({
        checkingUsername: false,
  ...
  generateSeed = () => {
    ...
    let masterKey = dpay.auth.getPrivateKeys(name, seed, ['master']);
    let password = 'P' + masterKey.master;
    const types = ['owner', 'active', 'posting', 'memo'];
    let pub = dpay.auth.generateKeys(name, password, types);
    let prv = dpay.auth.getPrivateKeys(name, password, types);
  ...
  render() {
    ...
    const availableFundingMethods = ['BEX','VESTS'].map((name) => {
      return name === 'BEX' ? {
        key:name,
        text:name,
        value:name
                  ...
                  <Grid.Column>
                    <Segment basic>
                      <Header>
                        ...
                        <Header.Subheader>
                          WRITE THIS DOWN (or save in a password manager). This password will allow you to log into any dPay related service with full permissions. Use the posting private key if you only wish to grant posting permissions.
                        </Header.Subheader>
                      </Header>
                      <Segment>
                        {this.state.password}
            ...
            <Grid.Column width={7}>
              <Message negative>
                <Message.Content>
                  ...
                  <List bulleted>
                    <List.Item>BEX: {delegation.fee.amount}</List.Item>
                    <List.Item>VESTS: {delegation.delegation.amount.toFixed(6)} (~{delegation.bp.amount.toFixed(3)} BP)</List.Item>
                  </List>
                </Message.Content>
              </Message>
              <Header size="small">
